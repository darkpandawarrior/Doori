 ...
 *   `data["path"]` to a deep link so the tap opens the right screen via [com.mileway.LauncherActivity]
 *   (which routes through the shared DeepLinkRouter). PendingIntent uses FLAG_IMMUTABLE (API 31+ requirement).
 */
class MilewayFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        val title = message.notification?.title ?: message.data["title"] ?: "Doori"
    ...
    private fun showNotification(
        ...
        val uri = path?.let { if (it.contains("://")) it else "mileway://$it" }
        val intent =
