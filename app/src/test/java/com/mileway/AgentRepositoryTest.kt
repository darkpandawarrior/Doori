...
class AgentRepositoryTest {
    ...
    @Test
    fun `seedIfEmpty seeds mock data on first run`() =
        runTest {
            val repo = repo()
            repo.seedIfEmpty()
    ...
    @Test
    fun `seedIfEmpty is idempotent on second call`() =
        runTest {
            val repo = repo()
            repo.seedIfEmpty()
            repo.seedIfEmpty()
    ...
    @Test
    fun `seeded conversations have unique ids`() =
        runTest {
            val repo = repo()
            repo.seedIfEmpty()
            repo.conversationsFlow.test {
    ...
    @Test
    fun `seeded conversations have non-blank titles`() =
        runTest {
            val repo = repo()
            repo.seedIfEmpty()
            repo.conversationsFlow.test {
    ...
    @Test
    fun `createThread and appendMessage persist through Room`() =
        runTest {
            val repo = repo()
    ...
    @Test
    fun `conversationsFlow emits after createThread`() =
        runTest {
            val repo = repo()
            repo.conversationsFlow.test {
    ...
    @Test
    fun `getResumableThread returns thread id within 5-minute window`() =
        runTest {
            val repo = repo()
    ...
    @Test
    fun `getResumableThread returns null after 5-minute window`() =
        runTest {
            val repo = repo()
            repo.setActiveThread("t1", 1000L)
    ...
    @Test
    fun `appendMessage updates lastMessageMs on the conversation`() =
        runTest {
            val repo = repo()
