...
class OfflineAssistantEngineTest {
    ...
    private suspend fun collectDone(userMsg: String): String {
        ...
        engine.respond("conv-1", userMsg, 0).test {
            while (true) {
                val item = awaitItem()
                if (item is AssistantChunk.Done) {
                    doneText = item.fullText
                    break
                }
            }
    ...
    @Test
    fun `mileage week reply reflects actual seeded trips`() =
        runTest {
    ...
    @Test
    fun `mileage week with no trips returns informative message`() =
        runTest {
    ...
    @Test
    fun `card balance reply reflects the real cards feature data, not a fictional balance`() =
        runTest {
    ...
    @Test
    fun `title suggestion set on first message`() =
        runTest {
    ...
    @Test
    fun `title suggestion null on follow-up messages`() =
        runTest {
    ...
    @Test
    fun `first chunk is always a Thinking chunk`() =
        runTest {
