...
private class ErrorAssistantEngine(
    private val reason: AiFailure,
) : AssistantEngine {
...
class AgentViewModelTest {
    ...
    private fun viewModel(engine: AssistantEngine = FakeAssistantEngine()) =
        AgentViewModel(
            AgentRepository(FakeAgentDao(), FakeAgentSessionStore()),
            engine,
            FakeSpeechToText(),
            FakeTextToSpeech(),
            FakeShareSheet(),
            FakeAgentAnalyticsStore(),
        )

    @Test
    fun `init seeds popular and unanswered tabs synchronously, history after coroutine`() =
        runTest {
            val vm = viewModel()
            // popularTab and unansweredTab are set synchronously in the constructor
            assertTrue(
                vm.state.value.popularTab
                    .isNotEmpty(),
            )
            assertTrue(
                vm.state.value.unansweredTab
                    .isNotEmpty(),
            )
    ...
    @Test
    fun `sendMessage appends the user message and streams an assistant reply`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `sendMessage creates a thread and sets activeThreadId`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `loadConversation loads messages from Room for that conversation`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `loadConversation cancels previous messages collection`() =
        runTest {
            val vm = viewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `a mid-stream AiFailure stops streaming and sets the error field, message stays in transcript`() =
        runTest {
    ...
    @Test
    fun `StopStreaming cancels the in-flight reply and clears streaming state without an assistant reply`() =
        runTest {
    ...
    @Test
    fun `RetryLastMessage resends the last user question and clears the error`() =
        runTest {
