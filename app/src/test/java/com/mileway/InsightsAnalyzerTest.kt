...
import com.mileway.feature.tracking.insights.ActivityAnalyzer
import com.mileway.feature.tracking.insights.ActivityType
import com.mileway.feature.tracking.insights.DistanceQualityAnalyzer
import com.mileway.feature.tracking.insights.JourneyQualityAnalyzer
import com.mileway.feature.tracking.insights.RouteAnalyzer
import com.mileway.feature.tracking.insights.SystemImpactAnalyzer
...
class InsightsAnalyzerTest {
    ...
    fun `score factors list matches applied deductions`() {
        val result =
            JourneyQualityAnalyzer().analyze(
    ...
    fun `harsh acceleration events are captured with correct type`() {
        ...
        val points =
            times.zip(speeds).mapIndexed { i, (t, s) ->
                makeLocation(18.5 + i * 0.001, 73.8, t, speed = s)
            }
        val result = ActivityAnalyzer().analyze(points)
        val harshAccelEvents =
            result.accelerationProfile.harshEvents
                .filter { it.type == com.mileway.feature.tracking.insights.AccelerationType.HARSH_ACCELERATION }
