...
class MileageSubmissionViewModelTest {
    ...
    @Test
    fun `init loads offices and entities from config`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `init populates two required form fields`() =
        runTest {
            val vm = viewModel()
            assertEquals(2, vm.state.value.form.fields.size)
            assertTrue(
                vm.state.value.form.fields
                    .all { it.required },
            )
        }

    @Test
    fun `canSubmit is false when required fields are empty`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `canSubmit is true when all required fields and office and entity are filled`() =
        runTest {
            val vm = viewModel()
            ...
            val office =
                vm.state.value.form.offices
                    .first()
            val entity =
                vm.state.value.form.entities
                    .first()
    ...
    @Test
    fun `SetFormValue stores value by field id`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `ToggleDraft updates saveAsDraft in form`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `SelectOffice stores matching office and closes picker`() =
        runTest {
            val vm = viewModel()
            val target =
                vm.state.value.form.offices
            ...
            assertEquals(
                target.code,
                vm.state.value.form.selectedOffice
                    ?.code,
            )
    ...
    @Test
    fun `SelectEntity stores matching entity and closes picker`() =
        runTest {
            val vm = viewModel()
            val target =
                vm.state.value.form.entities
            ...
            assertEquals(
                target.name,
                vm.state.value.form.selectedEntity
                    ?.name,
            )
            assertEquals(SubmissionSheet.NONE, vm.state.value.form.sheet)
        }
    ...
    @Test
    fun `DismissSheet resets active sheet to NONE`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `CaptureOdometerStart stores reading and image uri`() =
        runTest {
            val vm = viewModel()
            vm.onAction(
    ...
    @Test
    fun `D3 - a MANUAL-source capture flags the reading as manual`() =
        runTest {
            val vm = viewModel()
            vm.onAction(
                MileageSubmissionAction.CaptureOdometerStart(
    ...
    @Test
    fun `CaptureOdometerEnd computes odometerKm from start reading`() =
        runTest {
            val vm = viewModel()
            vm.onAction(
                MileageSubmissionAction.CaptureOdometerStart(
                    OdometerCaptureResult(
                        OdometerPurpose.START,
                        "uri://start",
                        reading = 50_000,
                        source = OdometerReadingSource.DEVICE_OCR,
                        captureTimeMs = 0L,
    ...
    @Test
    fun `submit success path emits Success state`() =
        runTest {
            val vm = viewModel()
            ...
            vm.onAction(
                MileageSubmissionAction.SelectOffice(
                    vm.state.value.form.offices
                        .first()
                        .code,
                ),
            ...
            vm.onAction(
                MileageSubmissionAction.SelectEntity(
                    vm.state.value.form.entities
                        .first()
                        .name,
                ),
    ...
    @Test
    fun `submit policy violation path parks response and shows violation sheet`() =
        runTest {
            val violatingApi =
            ...
            assertTrue(
                vm.state.value.form.violations
                    .isNotEmpty(),
            )
    ...
    @Test
    fun `ResolvePolicyAndFinalize transitions to Success after violation acknowledgement`() =
        runTest {
            val violatingApi =
                object : MilewayNetworkApi by FakeTrackingNetworkApi() {
                    override suspend fun submitMiles(request: SubmitMilesRequestK) =
    ...
    @Test
    fun `AddReceipt and RemoveReceipt update pendingReceipts in state`() =
        runTest {
            val vm = viewModel()
            ...
            assertEquals(
                "uri://receipt2",
                vm.state.value.pendingReceipts
                    .first(),
            )
        }

    @Test
    fun `Reset clears pending attachments and submission state`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `canSubmit is false when odometer is mandatory and not captured, fallback off`() =
        runTest {
            val vm =
                viewModel(
            ...
            assertTrue(!vm.state.value.form.canSubmit)
            assertTrue(
                vm.state.value.form.remainingRequirements
                    .contains("Odometer reading"),
            )
        }

    @Test
    fun `SetOdometerNotWorking unblocks canSubmit when fallback config is enabled`() =
        runTest {
            val vm =
                viewModel(
    ...
    @Test
    fun `submit with odometer-not-working fallback carries GPS distance and audit remark`() =
        runTest {
    ...
    private fun track(
        ...
        startTime = fixtureStartTimeMs,
        endTime = -1L,
        distance = 0.0,
        duration = 0L,
        selectedVehicleType = "fourWheelerPetrol",
        vehiclePricing = 10.0,
        createdAt = fixtureStartTimeMs,
        startedAtTimestamp = fixtureStartTimeMs,
        startedByEmployeeCode = "EMP001",
    )

    @Test
    fun `LoadTrackInfo marks form roundTrip true when start and end are close after a long trip`() =
        runTest {
    ...
    @Test
    fun `LoadTrackInfo leaves form roundTrip false when start and end are far apart`() =
        runTest {
    ...
    @Test
    fun `submit threads roundTrip into SubmitMilesRequestK`() =
        runTest {
