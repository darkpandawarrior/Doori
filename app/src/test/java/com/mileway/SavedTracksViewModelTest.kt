...
class SavedTracksViewModelTest {
    ...
    @Test
    fun `init loads tracks from repository`() =
        runTest {
    ...
    @Test
    fun `TabSelected switches active tab and clears selection`() =
        runTest {
            ...
            assertTrue(
                vm.state.value.selectedSubmissionIds
                    .isEmpty(),
            )
        }

    @Test
    fun `JourneySearchChanged updates journeySearch`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `JourneyFilterSelected updates active chip`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `SubmissionFilterSelected and SubmissionSourceSelected update chips`() =
        runTest {
            val vm = viewModel()
    ...
    @Test
    fun `SubmissionLongPressed enters selection mode with the id`() =
        runTest {
            dao.preload(makeTrack("T1", submitted = true))
            val vm = viewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `SubmissionSelectionToggled deselects when already selected`() =
        runTest {
            dao.preload(makeTrack("T1", submitted = true))
            val vm = viewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `CreateVoucher emits ack flag and clears selection`() =
        runTest {
            dao.preload(makeTrack("T1", submitted = true))
            val vm = viewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `VoucherAckConsumed clears the ack flag`() =
        runTest {
            dao.preload(makeTrack("T1", submitted = true))
            val vm = viewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `selection is pruned when a submitted track is removed from repository`() =
        runTest {
            dao.preload(makeTrack("T1", submitted = true))
