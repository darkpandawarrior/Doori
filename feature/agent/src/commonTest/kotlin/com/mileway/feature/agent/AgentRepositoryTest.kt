...
class AgentRepositoryTest {
    ...
    @Test
    fun `createThread inserts conversation in DAO`() =
        runTest {
    ...
    @Test
    fun `appendMessage inserts message under correct conversation`() =
        runTest {
            val (repo, dao) = buildRepo()
    ...
    @Test
    fun `messagesFor returns only messages for requested conversation`() =
        runTest {
            val (repo, dao) = buildRepo()
            repo.createThread("t1", "T", 1_000L)
    ...
    @Test
    fun `getResumableThread returns null when no active thread`() =
        runTest {
            val (repo) = buildRepo()
    ...
    @Test
    fun `getResumableThread returns thread id within resume window`() =
        runTest {
            val (repo) = buildRepo()
    ...
    @Test
    fun `persistFeedback updates message rating in DAO`() =
        runTest {
            val (repo, dao) = buildRepo()
            repo.createThread("t1", "T", 1_000L)
    ...
    @Test
    fun `seedIfEmpty does not crash when no conversations exist`() =
        runTest {
            val (repo) = buildRepo()
            repo.seedIfEmpty()
        }
