...
sealed interface TrackMilesAction {
    ...
    data class SetVehicleQuery(
        val query: String,
    ) : TrackMilesAction

    data class PickVehicle(
        val key: String,
    ) : TrackMilesAction
    ...
    data class SetVendorQuery(
        val query: String,
    ) : TrackMilesAction

    data class PickVendor(
        val id: String,
    ) : TrackMilesAction
    ...
    data class SetPauseReason(
        val reason: String?,
    ) : TrackMilesAction

    data class SetPauseCustomReason(
        val text: String,
    ) : TrackMilesAction

    data class ConfirmPause(
        val reason: String,
    ) : TrackMilesAction

    data object OpenResumeSheet : TrackMilesAction

    data class SetResumeNotes(
        val notes: String,
    ) : TrackMilesAction
